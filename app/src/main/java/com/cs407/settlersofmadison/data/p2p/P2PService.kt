...
class P2PService(
    ...
    // NEW: whether we currently have a peer connected
    ...
    fun host(port: Int = 8989) {
        ...
        scope.launch(Dispatchers.IO) {
            try {
                ...
                // If we're already closing intentionally, don't mark as Error
                if (!closed.get()) {
    ...
    private fun onSocketReady(s: Socket) {
        ...
        // NEW: as soon as we have a socket, we know we have a peer
        scope.launch(Dispatchers.Main) { _peerConnected.value = true }
    ...
    fun send(text: String) {
        ...
        scope.launch {
            try {
                ...
                // don't auto-close on send error
